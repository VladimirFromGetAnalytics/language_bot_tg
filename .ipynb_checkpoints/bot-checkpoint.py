...
async def topic_handler(message: types.Message, state: FSMContext):
    ...
    try:
        ...
        kb.add(KeyboardButton(text="🏠 Start over"))
...
async def generate_first_question(language, level, topic):
    prompt = (
        f"You are having a natural conversation in {language} with user. Both of you are playing roles relevant to the topic - or example, if the topic is 'Ordering food at a restaurant' you are a waiter and the user is a guest. The user has {level} level {language} and your current topic is {topic}."
...
async def continue_dialogue(language, level, topic, history, user_message):
    ...
    messages = [
        {"role": "system", "content": f"You are having a natural conversation in {language} with user."
...
async def main():
    try:
        ...
        commands = [
            types.BotCommand(command="start", description="🚀 Start language practice"),
            types.BotCommand(command="analyze", description="📊 Analyze your mistakes")
        ]
